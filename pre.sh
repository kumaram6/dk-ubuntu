...
if [[ $param_parttype == 'efi' ]]; then
    run "Installing kernel for  ${param_ubuntuversion} (~10 min)" \
        ...
        if [ \"${PKG_REPO_SEC_LIST}\" != \"\" ]; then echo \"deb ${param_mirror} ${param_ubuntuversion}-security ${PKG_REPO_SEC_LIST}\" | cat - /target/root/etc/apt/sources.list > /tmp/out && mv /tmp/out /target/root/etc/apt/sources.list; fi && \
        if [ \"${PKG_REPO_LIST}\" != \"\" ]; then echo \"deb ${param_mirror} ${param_ubuntuversion} ${PKG_REPO_LIST}\" | cat - /target/root/etc/apt/sources.list > /tmp/out && mv /tmp/out /target/root/etc/apt/sources.list; fi && \
        cp etc/apt/preferences.d/priorities /target/root/etc/apt/preferences.d/priorities && \
